...
package org.wso2.raspberrypicodebased.plugin.impl.dao.impl;
...
public class DeviceTypeDAOImpl {
    ...
    public Device getDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String selectDBQuery =
                    "SELECT raspberrypicodebased_DEVICE_ID, DEVICE_NAME" +
                            " FROM raspberrypicodebased_DEVICE WHERE raspberrypicodebased_DEVICE_ID = ?";
            ...
            if (resultSet.next()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("raspberrypicodebased device " + deviceId + " data has been fetched from " +
                            "raspberrypicodebased database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while fetching raspberrypicodebased device : '" + deviceId + "'";
            log.error(msg, e);
    ...
    public boolean addDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("raspberrypicodebased device " + device.getDeviceIdentifier() + " data has been" +
                            " added to the raspberrypicodebased database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while adding the raspberrypicodebased device '" +
                    device.getDeviceIdentifier() + "' to the raspberrypicodebased db.";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
    ...
    public boolean updateDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String updateDBQuery =
                    "UPDATE raspberrypicodebased_DEVICE SET  DEVICE_NAME = ? WHERE raspberrypicodebased_DEVICE_ID = ?";
            ...
            String msg = "Error occurred while modifying the raspberrypicodebased device '" +
    ...
    public boolean deleteDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String deleteDBQuery =
                    "DELETE FROM raspberrypicodebased_DEVICE WHERE raspberrypicodebased_DEVICE_ID = ?";
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("raspberrypicodebased device " + deviceId + " data has deleted" +
                            " from the raspberrypicodebased database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while deleting raspberrypicodebased device " + deviceId;
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, null);
    ...
    public List<Device> getAllDevices() throws DeviceMgtPluginException {
        ...
        try {
            ...
            String selectDBQuery =
                    "SELECT raspberrypicodebased_DEVICE_ID, DEVICE_NAME " +
                            "FROM raspberrypicodebased_DEVICE";
            stmt = conn.prepareStatement(selectDBQuery);
            resultSet = stmt.executeQuery();
            ...
            if (log.isDebugEnabled()) {
                log.debug("All raspberrypicodebased device details have fetched from raspberrypicodebased database.");
            }
            return iotDevices;
        } catch (SQLException e) {
            String msg = "Error occurred while fetching all raspberrypicodebased device data'";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, resultSet);
